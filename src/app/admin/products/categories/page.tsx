...
import { ArrowLeft, Plus, Edit, Trash2, Check, X, Loader2, Tag } from 'lucide-react';
...
export default function CategoriesPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const url = editing ? `/api/admin/products/categories/${editing.id}` : '/api/admin/products/categories';
  ...
  const toggleActive = async (cat: Category) => {
    const res = await fetch(`/api/admin/products/categories/${cat.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ is_active: !cat.is_active }),
    });
    if (res.ok) setCategories(prev => prev.map(c => c.id === cat.id ? { ...c, is_active: !cat.is_active } : c));
  };
    ...
    <div className="p-4 md:p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#2D1F1A]">Product Categories</h1>
            <p className="text-sm text-[#7A6A64]">{categories.length} categories</p>
          </div>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-[#E8DDD6] p-5 mb-5">
          <h2 className="font-semibold text-[#2D1F1A] mb-4">{editing ? 'Edit Category' : 'New Category'}</h2>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-[#E8DDD6] py-16 text-center">
          <Tag size={32} className="mx-auto mb-3 text-[#E8DDD6]" />
          <p className="text-sm text-[#7A6A64] mb-3">No categories yet</p>
        ...
        <div className="bg-white rounded-2xl border border-[#E8DDD6] overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[#F5EDE5]">
              {categories.map(cat => (
                <tr key={cat.id} className="hover:bg-[#FDF8F4] transition-colors">
                  <td className="px-5 py-4">
                    <p className="font-semibold text-[#2D1F1A] text-sm">{cat.name}</p>
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className="font-mono text-xs text-[#7A6A64]">{cat.slug}</span>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <span className="text-sm text-[#7A6A64] line-clamp-1">{cat.description ?? '—'}</span>
                  </td>
                  <td className="px-5 py-4">
                    <button onClick={() => toggleActive(cat)}
                      className={cn('w-8 h-4 rounded-full transition-colors relative flex-shrink-0',
                        cat.is_active ? 'bg-emerald-500' : 'bg-[#E8DDD6]')}>
